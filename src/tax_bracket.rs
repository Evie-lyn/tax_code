...
mod tests {
    ...
    fn test_monotonic_increase() {
        let ok_brackets = vec![Bracket(10.0, 0.1), Bracket(20.0, 0.2), Bracket(f64::INFINITY, 0.3)];
    ...
    fn test_income_exactly_at_bracket_boundary() {
        let brackets = vec![Bracket(10000.0, 0.01), Bracket(30000.0, 0.02), Bracket(f64::INFINITY, 0.03)];
        let tax_brackets = TaxBrackets::new(brackets);
        let income = 10000.0;
    ...
    fn test_high_income_hits_last_bracket() {
        ...
        let tax_brackets = TaxBrackets::new(brackets);
        let income = 50000.0;
        let expected_tax = (10000.0 * 0.01) + ((30000.0 - 10000.0) * 0.02) + ((50000.0 - 30000.0) * 0.03);
        let actual_tax = tax_brackets.taxes(income);
        let epsilon = 0.0001;
        assert!((actual_tax - expected_tax).abs() < epsilon);
    }
