...
fn generate_get_deductions(
    ...
    println!("cargo:rerun-if-changed={}", deductions_json_input_path.display());
    println!("cargo:rerun-if-changed={}", cargo_manifest_dir.join("src").join("states_step_deduction.json").display());
    ...
    let states_step_deduction_json_path = cargo_manifest_dir.join("src").join("states_step_deduction.json");
    let step_json_content = fs::read_to_string(&states_step_deduction_json_path)
        .map_err(|e| format!("Failed to read step deductions JSON from {}: {}", states_step_deduction_json_path.display(), e))?;
    let parsed_step_data: JsonStepDeductionData = serde_json::from_str(&step_json_content)
        .map_err(|e| format!("Failed to parse step deductions JSON from {}: {}", states_step_deduction_json_path.display(), e))?;
    ...
    for state_code in &all_states {
        ...
        if states_with_step_deductions.contains(state_code) { 
            let data = parsed_step_data.get(state_code.to_uppercase().as_str()).expect(&format!("State '{}' data not found in states_step_deduction.json but was expected. Ensure '{}' key exists at the top level.", state_code, state_code));
